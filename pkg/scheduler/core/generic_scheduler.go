...
func (g *genericScheduler) removeUnfitClusters(
	...
	for clusterName := range clusters {
		...
		res := g.scheduleFramework.RunFilterPlugins(ctx, placement, &spec.Resource, clusterObj.Cluster())
		if !res.IsSuccess() {
...
func (g *genericScheduler) findClustersThatFit(
	...
	for clusterName := range availableClusters {
		...
		res := fwk.RunFilterPlugins(ctx, placement, resource, clusterInfo.Cluster())
		if !res.IsSuccess() {
			klog.V(4).Infof("cluster %q is not fit", clusterName)
		} else {
